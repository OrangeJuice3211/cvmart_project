...
from networks.models import deeplabv3plus
...
model_urls = {'deeplabv3plus_xception': '/project/train/models/deeplabv3plus_xception_VOC2012_epoch46_all.pth'}
...
def main():
    ...
    writer = SummaryWriter('/project/train/models/' + NAME + 'log/')
    ...
    data_train_list = '/project/train/src_repo/dataset/kuaidi/Training_seg.txt'
    trainloader = data.DataLoader(MyDataSet_seg(data_train_root, data_train_list, crop_size=(w, h)),
                                  batch_size=BATCH_SIZE, shuffle=True, num_workers=16, pin_memory=True, drop_last=True)

    data_val_root = '/home/data/305/'
    data_val_list = '/project/train/src_repo/dataset/kuaidi/Validation_seg.txt'
    valloader = data.DataLoader(MyValDataSet_seg(data_val_root, data_val_list, crop_size=(w, h)), batch_size=1, shuffle=False, num_workers=16, pin_memory=True, drop_last=True)

    path = '/project/train/models/' + NAME
    ...
    for epoch in range(44,EPOCH):
        ...
        line_val = "val%d: vacc=%f, vdice=%f, vsensitivity=%f, vspecifity=%f, vjac=%f \n" % \
                (epoch, np.nanmean(vacc), np.nanmean(vdice), np.nanmean(vsen), np.nanmean(vspe), np.nanmean(vjac_score))
